...
def add_ttl_limit(data: dict, token_ttl: str):
    created_at = datetime.now(UTC).replace(tzinfo=None)  # Убираем timezone для совместимости с БД
...
async def issue_token(
    ...
    if token_types[token] == TokenTypes.refresh_token:
        ...
        await db.auth.make_session(session_id, int(payload['sub']), rT['iat'], rT['exp'], client.user_agent, client.ip,
                                   hashed_rT)
        return hashed_rT
...
async def issue_aT_rT(db: PgSqlDep, token_schema: TokenPayloadSchema):
    ...
    log_event(f'Выданы aT, rT токены для {repr(token_schema)}', level='WARNING')
    return encoded_aT, hashed_rT


async def reissue_aT(access_token: dict, refresh_token: str, db: PgSqlDep):
    sub, s_id, role_id = access_token.get('sub'), access_token.get('s_id'), access_token.get('role_id')
    ...
    if db_rT and secrets.compare_digest(db_rT['refresh_token'], refresh_token):
        ...
        new_access_token = await issue_token({'sub': sub, 's_id': s_id, 'role_id': role_id}, 'access_token')
